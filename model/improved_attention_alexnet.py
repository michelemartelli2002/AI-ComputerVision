...
# References: https://medium.com/@tahasamavati/squeeze-and-excitation-explained-387b5981f249
# From SENet (2017)
class SEBlock(nn.Module):
    def __init__(self, in_channel, ratio=16):
        super().__init__()
        # Squeeze the input in a single number, performing a global average pooling
        self.squeeze = nn.AdaptiveAvgPool2d(1)
        # Excitation layer
        self.excitation = nn.Sequential(
            # Squeeze to promote feature selection
            nn.Linear(in_channel, in_channel // ratio, bias=False),
            # Add non linearity
            nn.ReLU(inplace=True),
            # Expand back to size in_channel
            nn.Linear(in_channel // ratio, in_channel, bias=False),
            # Output values between [0,1] (attention weights)
            nn.Sigmoid()
        )

    def forward(self, x):
        batch_size, channel_count, _, _ = x.size()
        # Global average pooling: result shape [batch_size, channel_count]
        y = self.squeeze(x).view(batch_size, channel_count)
        # Apply excitation network: still shape [batch_size, channel_count]
        y = self.excitation(y).view(batch_size, channel_count, 1, 1)
        # Reshape to match input tensor

        # Scale the input tensor by the learned channel-wise weights
...
class AlexNetV3(pl.LightningModule):
...
def get_cifar10_model():
    ...
    return AlexNetV3(class_weights=cw,
                    lr=0.001,
                    num_classes=10)
